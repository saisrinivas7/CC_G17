...
struct Node {
    enum NodeType {
        BIN_OP, INT_LIT, STMTS, ASSN, DBG, IDENT, VAR_DECL, TERNARY_OP
    } type;
...
struct NodeVarDecl : public Node {
    std::string identifier;
    Node *expression;

    NodeVarDecl(std::string id, Node *expr);
    std::string to_string();
    llvm::Value *llvm_codegen(LLVMCompiler *compiler);
};
...
struct NodeTernary : public Node {
    ...
    NodeTernary(Node::NodeType type, Op op, Node *cond, Node *true_expr, Node *false_expr);
    std::string to_string();
    llvm::Value *llvm_codegen(LLVMCompiler *compiler);
};
